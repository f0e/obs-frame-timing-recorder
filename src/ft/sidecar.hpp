...
namespace ft::sidecar {
	...
	constexpr std::string_view SUFFIX = ".frametiming";
	...
	template<typename T>
	bool write_batch(std::ostream& out, std::string_view tag, std::span<const T> records) {
		if (records.empty())
			return true;
	...
	template<typename T>
	bool write_batch(std::ostream& out, std::string_view tag, const std::vector<T>& records) {
