...
public:
	...
	{
		uint64_t from = 0;
		return read_from(from, [](const T &) { return true; });
	}
...
template<typename T> std::vector<T> since(const Ring<T> &log, int64_t from, auto &&time_of)
{
	std::vector<T> out = log.snapshot();
	std::erase_if(out, [&](const T &item) { return (int64_t)std::invoke(time_of, item) < from; });
	return out;
}
